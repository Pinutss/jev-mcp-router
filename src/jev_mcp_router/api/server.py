"""Serveur HTTP stdlib : POST /v1/select et GET /healthz, /v1/llms."""
...
_FORBIDDEN_KEYS = {"api_key", "jev_api_key", "gateway_api_key"}
...
def handle_select(
    ...
    *,
    ...
    if any(str(key).lower() in _FORBIDDEN_KEYS for key in body):
    ...
    if not isinstance(query, str) or not isinstance(tools, list):
        raise ConfigurationError("query (string) et tools (array) sont obligatoires")
    selector = McpSelector(
        provider=settings.provider,
        settings=settings,
        gateway_provider=body.get("gateway_provider"),
        llm_prefer=body.get("llm_prefer"),
    )
    result = selector.select(
        query=query,
        tools=tools,
        required_permissions=body.get("required_permissions") or (),
        forbidden_permissions=body.get("forbidden_permissions") or (),
        max_tools=body.get("max_tools"),
        budget_tokens=body.get("budget_tokens"),
        min_relevance=body.get("min_relevance"),
        failed_tool_id=body.get("failed_tool_id"),
        allow_fallback=body.get("allow_fallback", True),
        scope=str(body.get("scope") or "default"),
        gateway_provider=body.get("gateway_provider"),
        gateway_model=body.get("gateway_model"),
        llm_prefer=body.get("llm_prefer"),
    )
