...
class Settings:
    ...
    def from_env(cls) -> Settings:
        return cls(
            ...
            min_relevance=_env_float("JEV_MCP_MIN_RELEVANCE", 0.15),
            max_tools=_env_int("JEV_MCP_MAX_TOOLS", 8),
            budget_tokens=_env_int("JEV_MCP_BUDGET_TOKENS", 800),
            max_candidates=_env_int("JEV_MCP_MAX_CANDIDATES", 50),
            redact_secrets=_env_bool("JEV_REDACT_SECRETS", True),
            host=_env("JEV_MCP_HOST", "127.0.0.1") or "127.0.0.1",
            port=_env_int("JEV_MCP_PORT", 8080),
            auth_token=_env("JEV_MCP_AUTH_TOKEN"),
            request_timeout=_env_float("JEV_MCP_TIMEOUT", 30.0),
            llm_prefer=_env("JEV_LLM_STRATEGY"),
        )
